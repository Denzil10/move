...
def get_client(creds):
    ...
    return Client(
        ...
        auth=get_oauth1_session(creds).auth,
    )
...
def upload_image(session, filepath):
    ...
    with open(filepath, "rb") as f:
        resp = session.post(
            MEDIA_UPLOAD_URL,
            files={"media": (os.path.basename(filepath), f, media_type)},
        )
...
def upload_video(session, filepath):
    ...
    with open(filepath, "rb") as f:
        while True:
            ...
            if resp.status_code not in (200, 201, 202, 204):
                print(
                    f"Video APPEND failed at segment {segment}: {resp.status_code} {resp.text}",
                    file=sys.stderr,
                )
                sys.exit(1)
            segment += 1
            ...
            print(
                f"  Uploaded {uploaded / 1024 / 1024:.1f}MB / {file_size / 1024 / 1024:.1f}MB"
            )
    print(f"Video APPEND complete: {segment} segments")

    # FINALIZE
    resp = session.post(
        MEDIA_UPLOAD_URL, data={"command": "FINALIZE", "media_id": media_id}
    )
    if resp.status_code not in (200, 201, 202):
    ...
    while processing_info and processing_info.get("state") not in ("succeeded", None):
        ...
        time.sleep(wait)
        resp = session.get(
            MEDIA_UPLOAD_URL, params={"command": "STATUS", "media_id": media_id}
        )
...
def cmd_post(args):
    ...
    if args.media:
        ...
        kind, _mime = detect_media_type(args.media)
